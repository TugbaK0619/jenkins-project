...
resource "aws_instance" "docker-machine-managers" {
  ...
  tags = {
    Name = "tugba-Docker-Swarm-Manager-${count.index + 1}"
...
resource "aws_instance" "docker-machine-workers" {
  ...
  tags = {
    Name = "tugba-Docker-Swarm-Worker-${count.index + 1}"
...
resource "aws_security_group" "tf-docker-sec-gr" {
  name = "tugba-docker-swarm-sec-gr"
  tags = {
    Name = "swarm-sec-gr"
  }
  dynamic "ingress" {
